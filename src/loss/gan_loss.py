...
class GANLoss(nn.Module):
    ...
    Args:
        ...
        loss_g_weight (float): Loss weight. Default: 1.0.
            Note that loss_g_weight is only for generators; and it is always 1.0
            for discriminators.
    """
    ...
    def __init__(self,
        ...
        if self.gan_type == 'vanilla':
            ...
            self.loss = nn.ReLU()
        elif self.gan_type == 'ns':
            self.loss = self._ns_loss
        else:
            raise NotImplementedError(
                f'GAN type {self.gan_type} is not implemented.')

    def _ns_loss(self, input, is_disc, target_is_real):
        if is_disc and not target_is_real:
    ...
    def forward(self, input, target_is_real, is_disc=False, mask=None):
        ...
        if self.gan_type == 'hinge':
            ...
            loss = self.loss(input, is_disc, target_is_real)
        else:  # other gan types
            loss = self.loss(input, target_label)

        # loss_d_weight is always 1.0 for discriminators
        weighted_loss = loss * self.weight_d if is_disc else loss * self.weight_g
